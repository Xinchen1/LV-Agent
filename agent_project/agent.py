...
class OpenMythosAgent:
    ...
    def _run_simple(self, task: str, memory_context: str = "",
        ...
        system_parts = [
            ...
            "- run_code(code, lang, timeout=30) → 执行 python/bash/python_file 代码",
    ...
    _TOOL_ALIASES = {
        ...
        "pthon": "python_exec", "python_exe": "python_exec",
    ...
    def _parse_tool_args(self, args_str: str) -> Dict[str, Any]:
        ...
        if not args_str:
            return arguments
        # Normalize common quote variants and collapse whitespace
        normalized = args_str.replace('`', '"').replace('“', '"').replace('”', '"').replace('‘', "'").replace('’', "'")
        normalized = re.sub(r'\s+', ' ', normalized).strip()

        # Convert alternating param="key" value="val" pairs into key=val key=val form
        if normalized.startswith('param='):
            converted_parts = []
            pair_re = re.compile(r'param="([^"]+)"\s+value="([^"]*)"')
            for m in pair_re.finditer(normalized):
                converted_parts.append(f'{m.group(1)}="{m.group(2)}"')
            if converted_parts:
                normalized = ' '.join(converted_parts)

        # JSON / Python dict literal first
        for candidate in [normalized, "{" + normalized + "}"]:
            try:
                parsed = ast.literal_eval(candidate)
                if isinstance(parsed, dict):
                    return self._sanitize_parsed_args(parsed)
            except (SyntaxError, ValueError):
                pass
            # json.loads 可处理 JSON 字符串转义(如 "command": "echo 'x'")
            try:
                ...
                pass
        # 若整个字符串是一个被引号包裹的 JSON dict 字符串(如 bash_exec('{"command":"..."}')),
        # 剥掉外层引号再解析
        if normalized.startswith(('"', "'")) and normalized.endswith(('"', "'")):
            inner = normalized[1:-1]
            if inner.strip().startswith("{"):
                for cand in [inner, inner.replace('\\"', '"')]:
                    try:
                        ...
                        continue
        # Shell-style tokenization for space-separated key=value pairs
        try:
            for token in shlex.split(normalized):
                ...
                arguments[k] = v
            if arguments:
                return self._sanitize_parsed_args(arguments)
        except ValueError:
            pass
        # Fallback: comma/semicolon/newline split
    ...
    def _sanitize_parsed_args(self, parsed: Dict[Any, Any]) -> Dict[str, Any]:
        """Ensure parsed argument keys are strings and values are hashable-safe (recursively)."""
        result: Dict[str, Any] = {}
        for k, v in parsed.items():
            if not isinstance(k, (str, int, float, bool)):
                continue
            result[str(k)] = self._sanitize_value(v)
        return result

    def _sanitize_value(self, value: Any) -> Any:
        """Recursively convert unhashable/nested values into JSON-safe forms."""
        if isinstance(value, set):
            return sorted([self._sanitize_value(v) for v in value], key=str)
    ...
    def _parse_all_output_actions(self, text: str) -> List[ToolCall]:
        """从模型输出中提取所有工具调用（支持 [TOOL:] 和 OpenCode 格式）。"""
        if not text:
            return []
        results = []
        # 匹配所有 [TOOL:name]...[/TOOL] 块
        pattern = re.compile(r'\[TOOL:(\w+)\](.*?)\[/TOOL\]', re.DOTALL)
        for m in pattern.finditer(text):
            name = m.group(1).strip()
            args_str = m.group(2).strip()
            arguments = self._parse_tool_args(args_str)
            results.append(ToolCall(tool_name=name, arguments=arguments))

        # 回退:模型经常省略 [/TOOL] 闭合标签,补匹配未闭合的 [TOOL:name] {...}
        # 只在严格模式没匹配到时启用,避免与已匹配的结果重复
        if not results:
            unclosed_pattern = re.compile(
                r'\[TOOL:(\w+)\]\s*(.*?)(?=\[TOOL:\w+\]|\Z)', re.DOTALL
            )
            for m in unclosed_pattern.finditer(text):
                name = m.group(1).strip()
                args_str = m.group(2).strip()
                # 去掉可能残留的 [/TOOL] 片段
                args_str = args_str.replace('[/TOOL]', '').strip()
                arguments = self._parse_tool_args(args_str)
                results.append(ToolCall(tool_name=name, arguments=arguments))

        # OpenCode / opencode serve wrapped function calls
        opencode_pattern = re.compile(
            r'<\|message_model\|>\s*([\w_]+)\s*<\|content_invoke_tool_json\|>\s*(.*?)\s*<\|end_message\|>',
            re.DOTALL
        )
        for m in opencode_pattern.finditer(text):
            tool_name = m.group(1).strip()
            payload_text = m.group(2).strip()
            payload = None
            first_brace = payload_text.find('{')
            if first_brace >= 0:
                for start_idx in range(first_brace, len(payload_text)):
                    candidate = payload_text[start_idx:]
            ...
            if isinstance(payload, dict):
                ...
                if isinstance(args, dict):
                    results.append(ToolCall(tool_name=tool_name, arguments=self._sanitize_parsed_args(args)))

        # JSON 对象含 action+args(模型新格式, 可能缺 [TOOL: 前缀):
        # { "thoughts": "...", "action": "file_ops", "args": {...} } [/TOOL]
        if not results:
            for m in re.finditer(r'\{\s*"thoughts"?\s*:.*?"action"\s*:\s*"(\w+)".*?"args"\s*:\s*(\{.*?\})\s*\}', text, re.DOTALL):
                tool_name = m.group(1)
        ...
        for m in func_pattern.finditer(text):
            ...
            if args_str:
                # 若参数是单个字符串(如 bash_exec("echo 'x'; du ...")), 不是 key=value:
                # 对 bash_exec/python_exec 这类, 直接当作 command/code 参数
                ...
                if single_str and tool_name in ("bash_exec", "python_exec", "run_code"):
                    ...
                    # Wrap in braces so _parse_tool_args handles it as key=value pairs
                    arguments = self._parse_tool_args(args_str)
            else:
                arguments = {}
            results.append(ToolCall(tool_name=tool_name, arguments=arguments))
